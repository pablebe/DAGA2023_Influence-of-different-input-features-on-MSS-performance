#%% Python script to train a modified Open-Unmix model [1] without random mixing. 
#   Allows the training of the CompSpec models from [2], which are all Open-Unmix models 
...
from openunmix.model_STFT import OpenUnmixComplex, make_filterbanks, ComplexNorm
...
RANDOM_MIX_FLAG = False  # if True => random mixing training strategy
...
config = toml.load(os.path.join(config_path, 'open_unmix_CompSpec_config.toml'))
...
def get_model_summary(FS: int, model, device):
    ...
    x = torch.randn([1,NB_CHANNELS,NBINS,int((FS)/NHOP),2]).to(device)
...
def get_statistics(encoder, dataset, verbose_Flag):
    encoder = copy.deepcopy(encoder).to("cpu")
    scaler_re = preprocessing.StandardScaler()
    scaler_im = preprocessing.StandardScaler()
    ...
    for ind in pbar:
        ...
        X = encoder(mix[None, ...]).mean(1, keepdim=False).permute(0, 2, 1, 3)


        scaler_re.partial_fit(np.squeeze(X[..., 0]))
        scaler_im.partial_fit(np.squeeze(X[..., 1]))

    # set inital input scaler values
    mean_re = scaler_re.mean_
    mean_im = scaler_im.mean_
    mean = np.stack((mean_re, mean_im),1)

    std_re = np.maximum(scaler_re.scale_, 1e-4 * np.max(scaler_re.scale_))
    std_im = np.maximum(scaler_re.scale_, 1e-4 * np.max(scaler_re.scale_))
    std = np.stack((std_re,std_im),1)
...
def run_valid_epoch(open_unmx_model: OpenUnmixComplex, encoder: torch.nn.Sequential, decoder: torch.nn.Sequential, dl_valid: DataLoader, loss: torch.nn.MSELoss, compute_metrics: bool, valid_epoch: int):
    
    valid_device = next(open_unmx_model.parameters()).device
    ...
    comp_Norm = ComplexNorm(mono= NB_CHANNELS == 1)

    for valid_batch_ct, valid_batches in enumerate(pbar:=tqdm.tqdm(dl_valid)):

        with torch.no_grad():
            mix_batch = valid_batches[0].to(valid_device)
            target_batch = valid_batches[2].to(valid_device)
            mix_batch_stft = encoder[0](mix_batch).to(valid_device)
            target_batch_stft = encoder[0](target_batch).to(valid_device)

        enh_batch_stft = open_unmx_model(mix_batch_stft)
        #inverse fourier transform for stft-consistency
        enh_batch_audio = decoder(enh_batch_stft)
        enh_batch_stft = encoder(enh_batch_audio)

        if LOSS_FN=='mse':
            target_batch_spec = comp_Norm(target_batch_stft+1e-16).to(valid_device)
            enh_batch_spec = comp_Norm(enh_batch_stft+1e-16)#.to(train_device)
            err = loss(target_batch_spec, enh_batch_spec)
        elif LOSS_FN=='ccmse':
        ...
        if compute_metrics:
            ...
            #stack estimates (vocals and residual)
            estimate_batch = torch.stack((enh_batch_audio,mix_batch-enh_batch_audio), dim=-1)
            ...
            cleanup(enh_batch_stft,enh_batch_audio,estimate_batch)
        else:
            cleanup(enh_batch_stft,enh_batch_audio)
...
def main():
    ...
    dl_train = DataLoader(mus_train, BATCH_SIZE_TRAIN, shuffle=True, num_workers=NUM_PROCESSES_TRAIN_LOADER,
        pin_memory=True, worker_init_fn = worker_init_fn_rand, drop_last = False, persistent_workers=False)
    ...
    encoder = torch.nn.Sequential(stft)
    decoder = torch.nn.Sequential(istft)
    comp_Norm = ComplexNorm(mono= NB_CHANNELS == 1)

    train_dataset_mu, train_dataset_std = get_statistics(encoder, mus_train, VERBOSE)
    max_bin = bandwidth_to_max_bin(FS, NFFT, BANDWIDTH)


    open_unmx_model = OpenUnmixComplex(nb_bins=NBINS, nb_channels=NB_CHANNELS, hidden_size=HIDDEN_SIZE, max_bin = max_bin, nb_layers=N_LAYERS, unidirectional=UNI_DIR_FLG, input_mean=train_dataset_mu, input_scale=train_dataset_std)
    ...
    for epoch in range(MAX_EPOCHS)[start_epoch:]:
        ...
        for batch_ct, batches in enumerate(pbar:=tqdm.tqdm(dl_train)):
            ...
            # calculate loss
            if LOSS_FN=='mse':
                target_batch_spec = comp_Norm(target_batch_stft+1e-16).to(train_device)
                enh_batch_spec = comp_Norm(enh_batch_stft+1e-16)#.to(train_device)    
                err = loss(target_batch_spec, enh_batch_spec)
            elif LOSS_FN=='ccmse':
                err = loss(target_batch_stft, enh_batch_stft)
            ...
            err.backward()
            torch.nn.utils.clip_grad_norm_(open_unmx_model.parameters(), 1.0, error_if_nonfinite=True)
            optimizer.step()
            ...
            if (batch_ct+1)%N_log_batches == 0 or batch_ct==0:
                if batch_ct>0:
                    ...
                    n_estimates = N_TARGETS+int(COMPUTE_RESIDUAL)
                    estimate_batch = torch.zeros((n_samples, n_channels, length, n_estimates))
                    estimate_batch[...,0] = enh_batch_audio.cpu()
                    estimate_batch[...,1] = mix_batch.cpu() - enh_batch_audio.cpu()

                    residual_batch = mix_batch.cpu()-target_batch.cpu()
        ...
        if LOSS_FN=='mse':
            cleanup(mix_batch, target_batch, mix_batch_stft, target_batch_stft, target_batch_spec, enh_batch_spec)
        elif LOSS_FN=='ccmse':
            cleanup(mix_batch, target_batch, mix_batch_stft, target_batch_stft)
