...
from .transforms import make_filterbanks, ComplexNorm, apply_complex_multiplication


class OpenUnmixComplex(nn.Module):
    ...
    def __init__(
        ...
        super(OpenUnmixComplex, self).__init__()
        ...
        #two times the original input-size because of Real & Imaginary part
        ...
        fc2_hiddensize = hidden_size * 2 # x2 because of the skip connection
        ...
        self.fc3 = Linear(
            ...
            out_features=self.nb_output_bins * nb_channels * 2, # x2 because of the complex dimension (concatenated real and imaginary masks)
            bias=False,
        )

        self.bn3 = BatchNorm1d(self.nb_output_bins * nb_channels * 2) # x2 because of the complex dimension (concatenated real and imaginary masks)

        if input_mean is not None:
            input_mean = torch.from_numpy(-input_mean[: self.nb_bins]).float()
        else:
            input_mean = torch.zeros(self.nb_bins,2)

        if input_scale is not None:
            input_scale = torch.from_numpy(1.0 / input_scale[: self.nb_bins]).float()
        else:
            input_scale = torch.ones(self.nb_bins,2)

        self.input_mean = Parameter(input_mean)
        self.input_scale = Parameter(input_scale)

        # dimension 1 is set to 2 because of Complex dimension
        self.output_scale = Parameter(torch.ones(self.nb_output_bins,2).float())
        self.output_mean = Parameter(torch.ones(self.nb_output_bins,2).float())
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        nb_frames, nb_samples, nb_channels, nb_bins, nb_complex = x.data.shape
        ...
        #stack real and imaginary part on top of each other
        x = torch.cat((x[...,0], x[...,1]),-1)

        # to (nb_frames*nb_samples, nb_channels*nb_bins)
        # and encode to (nb_frames*nb_samples, hidden_size)
        x = self.fc1(x.reshape(-1, nb_channels * self.nb_bins*2))# x2 because of the stacked real and imaginary part!
        ...
        x_re = x[...,:int(2*self.nb_output_bins)].reshape(nb_frames, nb_samples, nb_channels, self.nb_output_bins)
        x_im = x[...,int(2*self.nb_output_bins):].reshape(nb_frames, nb_samples, nb_channels, self.nb_output_bins)
        x = torch.stack((x_re,x_im),-1)
        ...
        x = apply_complex_multiplication(x,mix)#apply_complex_multiplication(F.relu(x),mix)#apply_complex_multiplication(F.leaky_relu(x,negative_slope=1),mix)


        #permute to (nb_samples, nb_channels,nb_bins,nb_frames,, nb_complex=2) ==> OLD COMMENT: # permute back to (nb_samples, nb_channels, nb_bins, nb_frames)
        return x.permute(1,2,3,0,4)#x.permute(1, 2, 3, 0)
