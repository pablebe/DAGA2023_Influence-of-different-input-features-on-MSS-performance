...
class MSSMUSDBDataset(data.Dataset):
    def __init__(self, foldername_list, target_str, random_mix_flag, augmentation_flag, duration, samples_per_track, fs, vcqt_config, feature_output) -> None:
        super().__init__()
        ...
        self.vcqt = VCQT(vcqt_config['fmin'], vcqt_config['fmax'], vcqt_config['nbins'], Qvar=vcqt_config['qvar'], fs=fs, audio_len=int(duration*fs), multichannel=True, device = torch.device("cpu"), split_0_nyq=False)
...
#            mixture, _ = soundfile.read(mixture_filepath, frames = seq_dur, start = mixture_start_index, dtype='float32')
    ...
    def __getitem__(self, item):
        ...
        if self.feature_output:
            mix = self.vcqt.fwd(temp[0].unsqueeze(0)).squeeze(0)
            if self.target_str[0]=='bass':
                target=self.vcqt.fwd(temp[1].unsqueeze(0)).squeeze(0)
                temp = (mix,target,temp[2],temp[3],temp[4],temp[5])
            elif self.target_str[0]=='vocals':
                target=self.vcqt.fwd(temp[2].unsqueeze(0)).squeeze(0)
                temp = (mix,temp[1],target,temp[3],temp[4],temp[5])
            elif self.target_str[0]=='other':
                target=self.vcqt.fwd(temp[3].unsqueeze(0)).squeeze(0)
                temp = (mix,temp[1],temp[2],target,temp[4],temp[5])
            elif self.target_str[0]=='drums':
                target=self.vcqt.fwd(temp[4].unsqueeze(0)).squeeze(0)
                temp = (mix,temp[1],temp[2],temp[3],target,temp[5])
