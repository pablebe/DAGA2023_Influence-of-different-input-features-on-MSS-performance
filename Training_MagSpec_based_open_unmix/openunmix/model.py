...
class OpenUnmix(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if False:
            fig = plt.figure()
            plt.pcolormesh(F.relu(x)[:,0,0,:].detach().cpu().T)

        x = F.relu(x) * mix


        #permute to (nb_samples, nb_frames, nb_bins, nb_channels) ==> OLD COMMENT: # permute back to (nb_samples, nb_channels, nb_bins, nb_frames)
        return x.permute(1,0,3,2)#x.permute(1, 2, 3, 0)


class OpenUnmixLog(nn.Module):
    """OpenUnmix Core spectrogram based separation module.
    ...
    def __init__(
        ...
        super(OpenUnmixLog, self).__init__()
