...
def apply_complex_multiplication(mask, mix):

    real = (mask[...,0]*mix[...,0]) - (mask[...,1]*mix[...,1])
    imag = (mask[...,0]*mix[...,1])+(mask[...,1]*mix[...,0])
    masked_mix = torch.stack((real,imag),-1)

    return masked_mix

def make_filterbanks(n_fft=4096, n_hop=1024, center=False, sample_rate=44100.0, method="torch", length=None):
    ...
    if method == "torch":
        ...
        decoder = TorchISTFT(n_fft=n_fft, n_hop=n_hop, window=window, center=center, length=length)
...
class TorchSTFT(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        stft_f = complex_stft #torch.view_as_real(complex_stft) => not needed if return_complex-Flag is used in torch.stft()
        # unpack batch
...
class TorchISTFT(nn.Module):
    ...
    def forward(self, X: Tensor, length: Optional[int] = None) -> Tensor:
        shape = X.size()
        X = X.reshape(-1, shape[-3], shape[-2], shape[-1]).contiguous()
...
class ComplexNorm(nn.Module):
    ...
    def __init__(self, mono: bool = False, log_flag: bool=False):
    ...
    def forward(self, spec: Tensor) -> Tensor:
        ...
        spec = (spec**2).sum(-1).sqrt()# torch.abs(torch.view_as_complex(spec)) ==> torch.view_as_complex() is not implemented on mps device!
