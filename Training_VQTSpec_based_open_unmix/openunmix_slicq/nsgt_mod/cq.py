...
class NSGT:
    def __init__(self, scale, fs, Ls, Qvar=1, real=True, matrixform=False, reducedform=0, multichannel=False, measurefft=False, multithreading=False, dtype=torch.float32, device="cpu"):
        assert fs > 0
        assert Ls > 0
        ...
        self.g,rfbas,self.M = nsgfwin(self.frqs, self.q, self.fs, self.Ls, Qvar=Qvar, sliced=False, dtype=dtype, device=self.device)

        # import matplotlib.pyplot as plt
        # plt.figure()
        # plt.plot(self.g[0])
        # plt.plot(self.g[1],linestyle='dashed')
